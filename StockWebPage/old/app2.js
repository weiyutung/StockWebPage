  ...
  const optionsPrice = {
    ...
    yaxis: [
      ...
      },
      {
        ...
        title: { text: "Bias" },
        labels: { formatter: (val) => Number(val.toFixed(2)) },
        opposite: true, // 放右側，避免擠在一起
        show: false, // 有勾 Bias 才顯示
      },
      {
        // 2: MACD (DIF/DEA)
        title: { text: "MACD" },
  ...
  document.querySelectorAll(".indicator-check").forEach((checkbox) => {
    checkbox.onchange = () => {
      ...
      checked.forEach((name) => {
        ...
        // 預設：SMA 走價格軸(0)
        let yAxisIndex = 0;
        if (indicatorGroups.bias.includes(name)) yAxisIndex = 1; // Bias 軸
        else if (indicatorGroups.macd.includes(name)) yAxisIndex = 2; // MACD 軸
        else if (indicatorGroups.kdj.includes(name)) yAxisIndex = 3; // KDJ 軸
      ...
      chart.updateOptions(
        {
          ...
          yaxis: [
            { ...chart.w.config.yaxis[0], show: true }, // 價格軸永遠顯示
            { ...chart.w.config.yaxis[1], show: showBias }, // Bias 軸
            { ...chart.w.config.yaxis[2], show: showMacd }, // MACD 軸
            { ...chart.w.config.yaxis[3], show: showKdj }, // KDJ 軸
          ],
        },
        false,
        false
