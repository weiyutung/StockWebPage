...
function highlightConditions(rules) {
  ...
  if (rules.length > 0) {
    window.stockData.forEach((row, i) => {
      ...
      function makePoint(i, row, text) {
        const low = parseFloat(row.low);
        // 為了保證一定在 Y 軸範圍內，可以用 low，或 low * 0.99 就好
        const y = low * 0.99;

        return {
          x: window.tradingDates[i], // 用類別軸的日期字串即可
          y: y,
          yAxisIndex: 0, // ⬅⬅⬅ 超重要：指定用「價格軸」
          marker: {
            size: 5,
            fillColor: "#000000",
            strokeColor: "#000000",
      ...
      if (rules.length === 1) {
        if (checks[rules[0]] && checks[rules[0]]()) {
          newAnnotations.push(makePoint(i, row, labelMap[rules[0]]));
        }
      } else {
        const allPass = rules.every((r) => checks[r] && checks[r]());
        if (allPass) {
          newAnnotations.push(
            makePoint(i, row, rules.map((r) => labelMap[r]).join(""))
          );
        }
      }
    });
  ...
  chart.updateOptions({
    annotations: {
      xaxis: existingXaxis, // 保留所有 X 軸標註 (包含區隔線)
...
function togglePeriods() {
  ...
  if (showPeriods) {
    // 打開區隔：呼叫 addPeriodSeparators (它現在會保留條件標註)
    addPeriodSeparators(currentMonths);
  } else {
    // 關閉區隔：只移除 period 相關，保留條件判斷 (highlight-marker)
    const existing = chart.w.config.annotations || {};
