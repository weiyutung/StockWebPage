...
function renderConditionRows() {
  ...
  conditionRows.forEach((row) => {
    ...
    rowEl.innerHTML = `
      ...
      <input type="number" class="value-input" placeholder="請輸入數值" />
...
function applyConditionBuilder(silent = false) {
  ...
  // 1. 讀取 AND / OR 選項
  const logicSelect = document.getElementById("globalLogic");
  const globalLogic = (logicSelect?.value || "AND").toUpperCase(); // 預設 AND
...
async function toggleSignalMarkers() {
  ...
  try {
    ...
    rowsInRange.forEach((row) => {
      ...
      const xCat = window.tradingDates[idx]; // 圖上真的那一天
