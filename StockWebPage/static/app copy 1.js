...
async function displayStockData(data, symbol) {
  ...
  window.updateIndicatorsFromChecked = () => {
    ...
    // 1) 先放主 K 棒（用 currentCandleData，可能已經附加未來 30 天）
    let newSeries = [
...
// 依日期抓出當天的「最低價下面一點」(low * 0.98)
// 這版用 Date 來比，避免格式有小差異對不到
...
function applyConditionBuilder(silent = false) {
  ...
  const preservedPoints = existingPointsRaw.filter((p) => {
    const css = p.label?.cssClass || "";
    // 保留「時間區隔標籤」和「買賣點標籤」
    return css.includes("period-label") || css.includes("signal-marker");
  });
...
async function toggleSignalMarkers() {
  ...
  // ===== 已經有買賣點 → 這次按就是把它們「一個一個 remove」掉 =====
  if (window.signalAnnoIds && window.signalAnnoIds.length > 0) {
    window.signalAnnoIds.forEach((id) => {
      try {
        window.priceChartInst.removeAnnotation(id);
      } catch (e) {
        console.warn("移除買賣點 annotation 失敗：", id, e);
      }
    });
    window.signalAnnoIds = [];
    if (btn) btn.classList.remove("active");
    console.log("[toggleSignalMarkers] 已移除買賣點");
    return;
  }

  // ===== 還沒有買賣點 → 去撈 DB 然後用 addPointAnnotation 畫上去 =====
  const symbol = getSymbol(); // 你原本就有這個函式
  ...
  try {
    ...
    const rows = await resp.json(); // e.g. [{ date: "2023-11-24", sig: "Buy" }, ...]
    ...
    if (rowsInRange.length === 0) {
      const firstSig = rows[0].date;
      const lastSig = rows[rows.length - 1].date;
      alert(
        `有買賣訊號，但不在目前顯示的區間內。\n\n` +
          `訊號日期範圍：${firstSig} ~ ${lastSig}\n` +
          `目前圖表區間：${minDate} ~ ${maxDate}\n\n` +
          `請試著切換成 6m / 1y / 3y，或用自訂日期把訊號日期包含進來。`
      );
      return;
    }
    ...
    rowsInRange.forEach((row, idx) => {
      const sig = row.sig;
      if (sig !== "Buy" && sig !== "Sell") return; // Hold 不顯示

      const dateStr = row.date; // "YYYY-MM-DD"
      const yVal = getLowPriceBelowByDate(dateStr);
      if (yVal == null) {
        console.warn("該日期沒有對應的 K 線，略過:", dateStr);
        return;
      }

      const isBuy = sig === "Buy";
      const color = isBuy ? "#00C853" : "#D50000"; // 綠=Buy, 紅=Sell
      const shape = isBuy ? "circle" : "square";
      const id = `sig-${dateStr}-${sig}-${idx}`;

      // ✅ 直接用 addPointAnnotation 畫上去
      try {
        window.priceChartInst.addPointAnnotation({
          id,
          x: dateStr, // X 軸是類別字串 → 跟 categories 一樣
          y: yVal, // 放在最低價下方一點
          yAxisIndex: 0, // 走價格那一條 Y 軸
          marker: {
            size: 6,
            fillColor: color,
            strokeColor: "#ffffff",
          ...
          label: {
            ...
            cssClass: "signal-marker", // 之後別的程式要保留它可以靠這個
          },
        });
        window.signalAnnoIds.push(id);
      } catch (e) {
        console.error("addPointAnnotation 失敗：", id, e);
      }
    });

    if (window.signalAnnoIds.length === 0) {
      alert("有找到訊號，但這個區間內對不到任何 K 線");
      return;
    }

    if (btn) btn.classList.add("active");
    console.log(
      `[toggleSignalMarkers] 已加上買賣點，共 ${window.signalAnnoIds.length} 個`
    );
  } catch (err) {
