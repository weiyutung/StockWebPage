...
async function displayStockData(data, symbol) {
  ...
  //   // 1) 主 K 線（用 currentCandleData，可能有未來 30 天）
  //   let newSeries = [
  ...
  //   // 4) 疊上「進階條件」的點（用 scatter）
  //   if (conditionMarkPoints.length > 0) {
  //     newSeries.push({
  //       name: "條件點",
  //       type: "scatter",
  //       data: conditionMarkPoints, // 每個點 { x, y, label }
  //       yAxisIndex: 0,
  //       markers: {
  //         size: 5,
  //         shape: "circle",
  ...
  window.updateIndicatorsFromChecked = () => {
    ...
    // === 8) 這裡統一設定「哪幾條要有 marker / label」 ===

    // 每條 series 的 marker size：其他都 0，只有我們的點>0
    const markerSizeArray = newSeries.map((s) => {
      if (s.name === "條件點") return 5;
      if (s.name === "Buy" || s.name === "Sell") return 6;
      return 0; // K 線和技術線不要 marker
    });
    ...
    // === 9) 更新下方 Volume padding（照你原本的） ===
    const rightAxisCount =
      (showMacd ? 1 : 0) + (showKdj ? 1 : 0) + (showBias ? 1 : 0);
    const axisWidth = 55;
    const baseVolRightPad = -25;
    const newVolRightPad = baseVolRightPad + rightAxisCount * axisWidth;
...
function applyConditionBuilder(silent = false) {
  ...
  // 若沒有符合條件的點 → 在最後一根放一個灰色提示點（文字用 label）
...
// =============================
// 買賣點 toggle：future30Btn2（改成用 scatter）
// =============================
async function toggleSignalMarkers() {
  ...
  // ===== 還沒有買賣點 → 去撈 DB，再丟給 scatter series =====
  const symbol = getSymbol();
  ...
  try {
    ...
    rowsInRange.forEach((row) => {
      ...
      if (sig === "Buy") {
        buyPts.push({ x: dateStr, y: yVal });
      } else if (sig === "Sell") {
        sellPts.push({ x: dateStr, y: yVal });
      }
    });
    ...
    console.log(
      `[toggleSignalMarkers] 已加上買賣點，Buy: ${buyPts.length} 個, Sell: ${sellPts.length} 個`
    );
  } catch (err) {
