  ...
  document.querySelectorAll(".indicator-check").forEach((checkbox) => {
    checkbox.onchange = () => {
      ...
      checked.forEach((name) => {
        ...
        // 確認 J 是否有正確的數據
        if (name === "J") {
          console.log(`J Line Data:`, dataSeries);
        }

        // 每條技術指標對應不同的 y 軸
        let yAxisIndex = 0; // 預設綁定在價格軸
        if (indicatorGroups.bias.includes(name)) yAxisIndex = 1; // Bias
        else if (indicatorGroups.macd.includes(name)) yAxisIndex = 2; // MACD
        else if (indicatorGroups.kdj.includes(name)) yAxisIndex = 3; // KDJ

        newSeries.push({
          name,
          type: "line",
          data: dataSeries,
          yAxisIndex, // 確保分配到正確的 y 軸
